...
// CompanyAttachmentsAdd — the design's dashed "Drop file · + link" control at the
// bottom of the Attachments card, wired to addAttachmentLink with
// parent_type='company'. Mirrors the approved contacts AttachmentsAdd.
export function CompanyAttachmentsAdd({ companyId }: { companyId: string }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await addAttachmentLink('company', companyId, { title: title.trim(), url: url.trim() });
        setTitle('');
        setUrl('');
        setOpen(false);
        setFileHint(false);
