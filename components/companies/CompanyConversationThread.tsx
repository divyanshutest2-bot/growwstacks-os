...
import { sendCompanyMessage } from '@/lib/actions/companies';
import { avatarBg, initials } from '@/lib/ui';
import type { ConversationRow } from '@/lib/types';

// CompanyConversationThread — the account-level conversation card for the company
// cockpit. Identical structure to the approved contacts ConversationThread, but
// sends via sendCompanyMessage (parent_type='company'). Records intent locally;
// it does NOT deliver to the external channel (n8n sync, Phase 2).
...
export function CompanyConversationThread({
  companyId,
  companyName,
  entries,
}: {
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        await sendCompanyMessage(companyId, { channel: defaultChannel, body });
        router.refresh();
      } catch {
  ...
  for (const e of visible) {
    ...
    const sender = e.sender_contact_name ?? e.sender_user_name ?? companyName;
    ...
    <div className="card conv-card" data-testid="card-conversation">
      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--color-border-subtle)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          ...
          <span className="cnt">
            account-level · {entries.length} {entries.length === 1 ? 'message' : 'messages'} ·{' '}
