...
// DetailHeaderTitle — the cockpit header's inline-editable task title (H1). The
// parent wraps this in data-testid="task-field-title"; this renders the always-
// present <input> (styled as the .dh-title-row h1) plus a field-status indicator.
// Field-level auto-save on blur via updateTaskField('title'). RLS decides whether
// the save lands (admin/pm always; a developer only if they are an assignee).
//
...
export function DetailHeaderTitle({
  ...
  function save(next: string) {
    ...
    startTransition(async () => {
      try {
        await updateTaskField(taskId, 'title', next);
        setState('saved');
      } catch {
        setState('error');
      }
