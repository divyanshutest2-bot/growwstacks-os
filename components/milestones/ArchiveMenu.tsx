...
import { archiveMilestone } from '@/lib/actions/milestones';

// ArchiveMenu (milestones) — the ONLY destructive affordance, and it is ARCHIVE,
// not delete (CLAUDE.md rule 4). Behind a kebab menu + explicit confirm. Calls
// archiveMilestone (sets archived_at); on success returns to the list. RLS gates
// it (milestone update is admin/pm only).
export function ArchiveMenu({ milestoneId }: { milestoneId: string }) {
  ...
  function archive() {
    startTransition(async () => {
      await archiveMilestone(milestoneId);
        ...
        <div className="fixed inset-0 z-10 flex items-center justify-center bg-overlay p-6">
          <div className="w-full max-w-sm rounded-lg border border-border-subtle bg-surface p-6 shadow-lg">
            ...
            <p className="mt-2 text-sm text-ink-secondary">
              The milestone is hidden from lists but never deleted. It can be
              restored later.
            </p>
