...
// of the Attachments card. "+ link" opens an inline title+url form wired to the
// real addAttachmentLink action; the drop-file path opens the same form and shows
// a "file upload coming soon" note (the n8n Drive proxy is Phase 5), so the control
// is present and functional up to where it can be — never absent.
export function AttachmentsAdd({ contactId }: { contactId: string }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await addAttachmentLink('contact', contactId, { title: title.trim(), url: url.trim() });
        setTitle('');
        setUrl('');
        setOpen(false);
        setFileHint(false);
  ...
  if (!open) {
    return (
      <button type="button" className="attach-drop" onClick={() => openForm(true)} data-testid="attachment-add">
        <Upload size={14} />
        Drop file ·{' '}
        <span
