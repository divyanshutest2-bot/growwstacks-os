...
// DetailOwnersEditor — the design's .ostack owner avatar stack (with a user-status
// dot per owner) + a dashed .add control. Multi-owner via the contact_owners join.
// Preserves owner-add / owner-chip / owner-remove testids the smoke spec depends on.
export function DetailOwnersEditor({
  contactId,
  owners,
  userOptions,
}: {
  ...
  return (
    <div className="meta-row">
      <span className="k">Contact owners</span>
        ...
        <div className="ostack">
          {owners.map((o) => (
            ...
            >
              ...
              <span
                className="sdot"
                style={{ background: USER_STATUS_DOT[o.status ?? 'active'] ?? 'var(--color-status-active)' }}
              />
              <button
                type="button"
