...
// NotesAdd — the design's "+ Add" note affordance for the cockpit Notes card.
// Opens an inline title+body composer; saves via the universal createNote action.
export function NotesAdd({ contactId }: { contactId: string }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await createNote('contact', contactId, { title: title || null, body });
        setTitle('');
        setBody('');
        setOpen(false);
        router.refresh();
