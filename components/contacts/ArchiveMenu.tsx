...
// ArchiveMenu — the ONLY destructive affordance, and it is ARCHIVE, not delete
// (CLAUDE.md rule 4). Behind a kebab menu + an explicit confirm. Calls
// archiveContact (sets archived_at); on success returns to the list.
export function ArchiveMenu({ contactId }: { contactId: string }) {
        ...
        <div className="fixed inset-0 z-10 flex items-center justify-center bg-overlay p-6">
          <div className="w-full max-w-sm rounded-lg border border-border-subtle bg-surface p-6 shadow-lg">
            ...
            <p className="mt-2 text-sm text-ink-secondary">
              The contact is hidden from lists but never deleted. It can be
              restored later.
            </p>
