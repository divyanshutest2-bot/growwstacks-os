...
import { sendMessage } from '@/lib/actions/contacts';
import { avatarBg, initials } from '@/lib/ui';
import type { ConversationRow } from '@/lib/types';

// Channel → CSS var() dot color (design --ch-*). Meeting channels reuse phone.
...
// The channel-filter chips the design shows (All + a curated set).
...
export function ConversationThread({
  ...
  // Default composer channel: the contact's most recent inbound channel, else whatsapp.
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        await sendMessage(contactId, { channel: defaultChannel, body });
        router.refresh();
      } catch {
  ...
  // Render rows with day separators.
  ...
  for (const e of visible) {
    ...
    const sender = e.sender_contact_name ?? e.sender_user_name ?? contactName;
