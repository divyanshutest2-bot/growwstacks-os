...
// UserAttachmentsAdd — the design's dashed "Drop file · + link" control at the
// bottom of the Attachments card, wired to addAttachmentLink with
// parent_type='user'. Mirrors the approved contacts/projects AttachmentsAdd.
export function UserAttachmentsAdd({ userId }: { userId: string }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await addAttachmentLink('user', userId, { title: title.trim(), url: url.trim() });
        setTitle('');
        setUrl('');
        setOpen(false);
        setFileHint(false);
