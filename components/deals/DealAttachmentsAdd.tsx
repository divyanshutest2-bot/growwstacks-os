...
// DealAttachmentsAdd — the dashed "Drop file · + link" control at the bottom of
// the deal Attachments card. "+ link" opens an inline title+url form wired to the
// universal addAttachmentLink('deal', …); the drop-file path opens the same form
// with a "coming soon" note (file proxy is later). Mirrors contacts' AttachmentsAdd.
export function DealAttachmentsAdd({ dealId }: { dealId: string }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await addAttachmentLink('deal', dealId, { title: title.trim(), url: url.trim() });
        setTitle('');
        setUrl('');
        setOpen(false);
        setFileHint(false);
