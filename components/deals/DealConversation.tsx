...
import { sendDealMessage } from '@/lib/actions/deals';
import { initials } from '@/lib/ui';
import type { ConversationRow } from '@/lib/types';

// DealConversation — the CENTER negotiation-history thread for the Deal Detail
// cockpit. Mirrors contacts' ConversationThread but titled "Negotiation history"
// and wired to sendDealMessage (parent_type='deal'). Carries the card-conversation
// testid.
...
export function DealConversation({
  dealId,
  contactName,
  entries,
}: {
  ...
  const defaultChannel =
    [...entries].reverse().find((e) => e.direction === 'inbound')?.channel ?? 'gmail';
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        await sendDealMessage(dealId, { channel: defaultChannel, body });
        router.refresh();
      } catch {
        setDraft(body);
      }
  ...
  for (const e of visible) {
    ...
    const sender = e.sender_contact_name ?? e.sender_user_name ?? contactName ?? 'Them';

    if (
      MEETING_CHANNELS.has(e.channel) &&
      (e.meeting_recording_url || e.meeting_summary || e.duration_minutes)
    ) {
      rows.push(
    ...
    <div className="card conv-card" data-testid="card-conversation">
      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--color-border-subtle)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          ...
          <h3 style={{ fontFamily: 'var(--font-sans)', fontSize: 13, fontWeight: 700, whiteSpace: 'nowrap' }}>
            Negotiation history
          </h3>
          <span className="cnt">
            {entries.length} {entries.length === 1 ? 'message' : 'messages'} · {channelCount}{' '}
      ...
      <div className="composer">
        <button type="button" className="icon-btn" title="Channel" style={{ width: 30, height: 30 }}>
          <span
            style={{
              width: 8,
              height: 8,
