...
// DealNotesAdd — the "+ Add" note affordance for the deal cockpit Notes card.
// Inline title+body composer; saves via the universal createNote('deal', …).
export function DealNotesAdd({ dealId }: { dealId: string }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await createNote('deal', dealId, { title: title || null, body });
        setTitle('');
        setBody('');
        setOpen(false);
        router.refresh();
