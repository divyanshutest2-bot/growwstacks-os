...
// ProjectAttachmentsAdd — the design's dashed "Drop file · + link" control at the
// bottom of the Attachments card, wired to addAttachmentLink with
// parent_type='project'. Mirrors the approved contacts/companies AttachmentsAdd.
export function ProjectAttachmentsAdd({ projectId }: { projectId: string }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await addAttachmentLink('project', projectId, { title: title.trim(), url: url.trim() });
        setTitle('');
        setUrl('');
        setOpen(false);
        setFileHint(false);
